...
| Linux / QNAP NAS | 串口模式可用 | 需要模块暴露标准 Quectel AT 串口；不支持 Linux 下的原始 DJI 私有 USB 接口 |
...
Linux 版本通过标准 AT 串口工作，适用于已经暴露 `/dev/ttyUSB*` 或 `/dev/ttyACM*` 的 Quectel 模块。可使用 `./scripts/build-linux.sh amd64` 或 `./scripts/build-linux.sh arm64` 生成静态二进制。完整的 QNAP 部署与安全说明见 [docs/QNAP.md](docs/QNAP.md)。
