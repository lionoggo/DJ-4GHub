...
func (m *Manager) SMSListAllPDU() ([]string, error) {
	// Keep startup polling to unread messages. Listing every status also returns
	// module-sent messages on many Quectel firmwares, which is unsuitable for
	// event-driven forwarding and can create a forwarding loop.
	resp, err := m.ExecuteAT("AT+CMGL=0", 10*time.Second)
	if err != nil {
		return nil, err
	}
