...
2. 申请并发布 `im:message`、`im:message:send_as_bot`；若要转发通话录音，另需
   `im:resource` 文件资源权限。
...
在“自动接听”区域启用“将完成录音转发到飞书语音”后，服务会保留本地 WAV、再
转换为 Opus 并以飞书原生语音气泡发送。该功能只支持“企业应用机器人私聊”，不
适用于群机器人 Webhook。
...
4. 首先完成一次来电测试：确认自动接听、来电方能听到提示语、挂断后出现 WAV
   录音。
5. 最后启用 Telegram 录音附件转发或飞书语音转发，并再次测试。
...
5. 确认 Telegram 收到 WAV 附件或飞书收到可播放的语音气泡（如已开启相应转发）。
...
| --- | --- | --- |
| 新短信文本 | 已支持 | 已支持 |
| 通话录音 | WAV 附件已支持 | Opus 原生语音气泡已支持 |

飞书录音转发采用“转换为 Opus → 上传文件 → 获得 `file_key` → 发送 `audio` 消息”
的流程。文件大小受飞书限制，并会在发送完成后清理临时 Opus 文件。详见飞书的
[上传文件接口](https://open.feishu.cn/document/server-docs/im-v1/file/create) 与
